...
class EnvEmuModule(wishful_module.AgentModule):
    def __init__(self):
        ...
        if "/dev/ee" in ee_node_output:
            self.ee_sf_process = subprocess.Popen(['../../experimentation_tools/EnvEmu/connectEE.rb'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, shell=True)
            self.ee_write_process = subprocess.Popen(['../../experimentation_tools/EnvEmu/write_to_ee.rb'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, shell=True)
    ...
    def __ee_listen(self, stop_event):
        while not stop_event.is_set():
            out = self.ee_sf_process.stdout.readline().strip()
            if out != "":
                self.log.info("%s", out)
    ...
    def start_energy_harvester(self):
        ...
        streamer_on_cmd = "EE_MSG_CONFIG_STREAMER{msgt_am=250, EEMsgConfigStreamer_time_us=0, EEMsgConfigStreamer_DACid=1, EEMsgConfigStreamer_sample=3550, EEMsgConfigStreamer_harvesterCapacitor=0, EEMsgConfigStreamer_harvester=0, EEMsgConfigStreamer_outerLoopResistor=0, EEMsgConfigStreamer_maxDACvalue = 4095 }\n"
